...
?>
	<script language="javascript">
		function confirma_apagar($codigo,$oque){
			if(window.confirm("Deseja apagar esta família e todos os produtos nela cadastrados?")) self.location = 'apagar.php?cd=' + $codigo + '&oque=' + $oque + '&origem=<?=$_SERVER['SCRIPT_NAME']?>';
		}
		
	</script>
	<span class="celula"><B>Famílias de Produtos Cadastrados no Sistema</B></span><br><br>
	<table>
		<tr>
	...
	<table cellpadding="0" cellspacing="0" width="400" style="border: 2px solid; ">
		...
		$query = "SELECT cd FROM familias WHERE familia LIKE '%" . $_REQUEST["filtro"] . "%'";
		$result = mysql_query($query) or die($query . "Erro de conexão ao banco de dados: " . mysql_error());
		$quantidade = mysql_num_rows($result);
		
		$query = "SELECT cd, familia, ordem FROM familias WHERE familia LIKE '%" . $_REQUEST["filtro"] . "%' ORDER BY ordem" . $query_limit;
		$result = mysql_query($query) or die($query . "Erro de conexão ao banco de dados: " . mysql_error());
		if(mysql_num_rows($result) == 0){
			if(strlen($_REQUEST["filtro"]) == 0) echo('<tr><td colspan="4">Nenhuma Família Cadastrada</td></tr>');
			else echo('<tr><td colspan="4">Nenhuma Família Encontrada</td></tr>');
		}
		$i = 0;
		while($registro = mysql_fetch_assoc($result)){
			if($i == 0){
				...
				<tr bgcolor="#F6F6F6" onMouseOver="this.bgColor='#FFFFFF';" onMouseOut="this.bgColor='#F6F6F6';">
					<td class="editar"><a href="form_familia.php?cd=<?=$registro['cd']?>">$</a></td>
					<td><?=$registro["familia"]?></td>
					<td align="center"><?=$registro["ordem"]?></td>
					<td class="apagar" align="center"><a href="#" onClick="confirma_apagar(<?=$registro['cd']?>,'familias');">r</a></td>
				</tr>
				<?
				$i = 1;
			}
